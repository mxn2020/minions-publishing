...
description: Agent skills for working with Minions Publishing MinionTypes. Provides CRUD operations, CLI usage, and best practices for AI agents managing minions-publishing data.
---

# Minions Publishing Agent Skills

Skills for agents operating on the `minions-publishing` toolbox.
...
```bash
# TypeScript
pnpm add @minions-publishing/sdk
...
The `publishing` CLI provides basic project info and utilities:
...
```ts
import { customTypes } from '@minions-publishing/sdk/schemas';
...
```python
from minions_publishing.schemas import custom_types
...
- This agent only writes to `minions-publishing` — it reads from other toolboxes but never writes to them
